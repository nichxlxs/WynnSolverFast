...
function parseSolverOutput(output) {
    ...
    if (!progress) throw new Error('solver output did not contain a completed result');
    const poolSizes = {};
    ...
    const dominance = output.match(/dominance:\s*(off|exact|safe|legacy),\s*(\d+)\s*->\s*(\d+)\s*items/i);
    const topCount = Number(progress[3]);
    // An exhaustive search with no valid candidates is a completed result,
    // not malformed output. A non-empty top-N must still publish its score.
    if (topCount > 0 && !score) {
        throw new Error('solver output reported results without a best score');
    }
    return {
        checked,
        ...
        elapsedMs,
        timedOut: !!progress[5],
        bestScore: score ? Number(score[1]) : null,
        ...
        dominanceMode: dominance ? dominance[1].toLowerCase() : null,
        dominanceInputItems: dominance ? Number(dominance[2]) : null,
        dominanceOutputItems: dominance ? Number(dominance[3]) : null,
    };
}
