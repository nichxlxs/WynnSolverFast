...
| A8 | Non-lowered atree plans (`scaling_kind == "full"`) — **narrowed to two causes** | Trees the exporter cannot lower to cached/split | Was four causes; two are now lowered. A variable effect writing a **dotted key** (`damMult.Surge`) or a multiplier-map sub-key was refused out of caution, but Rust already handled it: `eval_var_effects` routes outputs through `merge_stat` exactly as the JS does, `merge_into` recombines the partitions the same way, and the dense lowering declines them (`nested_prefix`) so they take the validated Obj path. Verified rather than assumed — `var_effect_check` compares 17 synthetic effect lists bit-for-bit against the JS `atree_eval_stat_effects`, covering dotted outputs, accumulation, both orders of a non-stacking sub-key, max caps and the positive clamp. **Still `full`:** (1) a var key the *constant* partition also writes — the full pass interleaves const and var contributions in effect order while the split sums each partition separately, and float addition is not associative; (2) `var_has_prop_io`, where property mutations feed back into other effects' `translate` lookups and genuinely need the per-leaf recompute. |
